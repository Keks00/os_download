...
function rsa.genKey(override)
    --if fs.exists("/public.key") or fs.exists("/private.key") and not override then
    --    return false
    --end
    textutils.slowPrint("Setting up encrytion Keys")
    shell.run("os/modules/keyGen.lua")
    return true
end
function fl(x)
	if x < 0 then
...
function multi_divide(numer, denom)
	local n = #denom.comps
...
function multi_divide_wrap(numer, denom)
...
function rsa.encrypt(msg)
    encrypted = {}
    local f = io.open("/public.key", "r")
    pubKey = textutils.unserialize(f:read("*a"))
    f:close()
    for i = 1,math.ceil(#msg / (bits / byteSize)) do
        if  i == math.ceil(#msg / (bits / byteSize)) then
            chunk = msg:sub((i-1)*(bits/byteSize) + 1,(i-1)*(bits/byteSize)+(#msg % (bits / byteSize)))
        else
            chunk = msg:sub((i-1)*(bits/byteSize) + 1,i*(bits/byteSize))
        end
        local res = bytesToNumber(stringToBytes(chunk), bits, byteSize)
        encrypted[i] = crypt(pubKey, res)
    end
    return encrypted
end
...
function rsa.decrypt(encrypted)
    ...
    privKey = textutils.unserialize(f:read("*a"))
    f:close()
    sleep(0.1)
    for i = 1,#encrypted do
        decrypted = crypt(privKey, encrypted[i])
        local decryptedBytes = numberToBytes(decrypted, bits, byteSize)
        combined = combined..bytesToString(decryptedBytes)
    end
    return combined
end
